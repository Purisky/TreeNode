...
namespace TreeNodeSourceGenerator
{
    public partial class NodeAccessorSourceGenerator
    {
        ...
        private string GeneratePropertyAccessorPartialClass(INamedTypeSymbol nodeType, List<AccessibleMemberInfo> accessibleMembers)
        {
            ...
            GenerateAccessMethod(sb, nodeType, accessibleMembers, AccessOperation.TryGet);
        ...
        private void GenerateAccessMethod(StringBuilder sb, INamedTypeSymbol nodeType, List<AccessibleMemberInfo> members, AccessOperation operation)
        {
            ...
            sb.AppendLine("        {");
            if (operation == AccessOperation.TryGet)
            {
            ...
            sb.AppendLine($"            if (first.IsIndex) {{ {methodInfo.IndexErrorHandling} }}");
                        if (operation == AccessOperation.TryGet)
            {
            sb.AppendLine("            if (path.Parts.Length == index + 1)");
            }
            else
            { 
        ...
        private void GenerateSinglePathMemberAccess(StringBuilder sb, AccessibleMemberInfo member, AccessOperation operation)
        {
            switch (operation)
            {
                ...
                case AccessOperation.TryGet:
                    sb.AppendLine($"                        value = ({member.Name} is T _{member.Name}) ? _{member.Name} : default(T);");
        ...
        private void GenerateCollectionAccess(StringBuilder sb, AccessibleMemberInfo member, AccessOperation operation)
        {
            ...
            {
                AccessOperation.Get => $"return {member.Name}.GetValueInternal<T>(path.SkipFirst);",
                AccessOperation.TryGet => $"return {member.Name}.TryGetValueInternal<T>(ref path,index + 1, out value);",
                AccessOperation.Set when member.IsStructCollection => 
                    $"{member.Name}.SetValueInternalStruct(path.SkipFirst, value);return;",
                AccessOperation.Set => 
                    $"{member.Name}.SetValueInternalClass(path.SkipFirst, value);return;",
                _ => throw new ArgumentException($"Unknown operation: {operation}")
            };
        ...
        private string GetNestedAccessMethod(AccessibleMemberInfo member, AccessOperation operation)
        {
            ...
            {
                AccessOperation.Get => $"return {member.Name}.GetValueInternal<T>(path.SkipFirst);",
                AccessOperation.Set => $"{member.Name}.SetValueInternal<T>(path.SkipFirst, value);return;",
                AccessOperation.TryGet => $"return {member.Name}.TryGetValueInternal<T>(ref path,index + 1, out value);",
                _ => throw new ArgumentException($"Unknown operation: {operation}")
            };
            ...
            {
                AccessOperation.Get => $"return PropertyAccessor.GetValue<T>({member.Name}, path.SkipFirst);",
                AccessOperation.Set => $"PropertyAccessor.SetValue({member.Name}, path.SkipFirst, value);return;",
                AccessOperation.TryGet => $"return PropertyAccessor.TryGetValue<T>({member.Name}, path.SkipFirst, out value);",
                _ => throw new ArgumentException($"Unknown operation: {operation}")
            };
        ...
        private string GetMultiPathEmptyHandling(AccessOperation operation)
        {
            return operation switch
            {
                ...
                AccessOperation.TryGet => "", // TryGet doesn't need special handling for empty case
                _ => throw new ArgumentException($"Unknown operation: {operation}")
            };
        }
    }
